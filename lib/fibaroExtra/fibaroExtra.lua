...
_MODULES.base={ author = "jan@gabrielsson.com", version = '0.4', depends={}, 
  init = function()
    ...
    copy,equal = table.copy,table.equal

    if not table.maxn then 
      --Exported: returns size of table
      function table.maxn(tbl) local c=0 for _ in pairs(tbl) do c=c+1 end return c end
    end

    --Exported: checks if k is member of table and returns index (or false)
...
_MODULES.climate={ author = "jan@gabrielsson.com", version = '0.4',depends={'base'},
  init = function()
    ...
    --Exported: Returns mode - "Manual", "Vacation", "Schedule"
    function fibaro.getClimateMode(id)
      return (api.get("/panels/climate/"..id) or {}).mode
    end

--Exported: Returns the currents mode "mode", or sets it - "Auto", "Off", "Cool", "Heat"
...
--Exported:  Set zone to scheduled mode
...
--Exported: Set zone to manual, incl. mode, time ( secs ), heat and cool temp
...
--Exported: Set zone to vacation, incl. mode, start (secs from now), stop (secs from now), heat and cool temp
...
_MODULES.cron={ author = "jan@gabrielsson.com", version = '0.4', depends={'base'},
  init = function()
    ...
    do
      ...
      local NJOBS = 1
      --Exported: creates a cronjob, returns reference
      ...
      --Exported: removes a cronjob given reference
...
_MODULES.pubsub={ author = "jan@gabrielsson.com", version = '0.4', depends={'base','event'},
  init = function()
    ...
    if QuickApp then -- only subscribe if we are an QuickApp. Scenes can publish
      --Exported: subscribe to an event
...
_MODULES.qa={ author = "jan@gabrielsson.com", version = '0.4', depends={'base','debug','event'}, 
  init = function()
    ...
    --Exported: Set QA variable value
    ...
    --Exported: Returns all QA variables
    ...
    --Exported: checks if QA is enabled
    ...
    --Exported: Sets QA property value
    ...
    --Exported: Debug message with formatting
    function QuickApp.debugf(_,...) fibaro.debugf(nil,...) end
    --Exported: Trace message with formatting
    function QuickApp.tracef(_,...) fibaro.tracef(nil,...) end
    --Exported: Warning message with formatting
    function QuickApp.warningf(_,...) fibaro.warningf(nil,...) end
    --Exported: Error message with formatting
...
--Exported: Like self:updateView but with formatting. Ex self:setView("label","text","Now %d days",days)
    function QuickApp:setView(elm,prop,fmt,...)
      local str = format(fmt,...)
      self:updateView(elm,prop,str)
    end

--Exported: Get view element value. Ex. self:getView("mySlider","value")
...
--Exported: Change name of QA. Note, if name is changed the QA will restart
    function QuickApp:setName(name)
      if self.name ~= name then api.put("/devices/"..self.id,{name=name}) end
      self.name = name
    end

--Exported: Set log text under device icon - optional timeout to clear the message
...
--Exported: Disable QA. Note, difficult to enable QA...
...
--Exported: Add interfaces to QA. Note, if interfaces are added the QA will restart
