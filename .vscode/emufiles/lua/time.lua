local function sunturnTime(date, rising, latitude, longitude, zenith, local_offset)
  ...
  local function fit_into_range(val, min, max)
    ...
    if val < min then
      count = floor((min - val) / range) + 1; return val + count * range
    elseif val >= max then
      count = floor((val - max) / range) + 1; return val - count * range
    else
      return val
    end
  end
  ...
  if rising then
    t = n + ((6 - lng_hour) / 24)                -- Rising time is desired
  else
    t = n + ((18 - lng_hour) / 24)
  end                                            -- Setting time is desired
  ...
  local cosH = (cos(zenith) - (sinDec * sin(latitude))) /
  (cosDec * cos(latitude))                                                           -- Calculate the Sun^s local hour angle
  if rising and cosH > 1 then
    return "N/R"                                                                     -- The sun never rises on this location on the specified date
  elseif cosH < -1 then
    return "N/S"
  end                                                                                -- The sun never sets on this location on the specified date
  ...
  ---@diagnostic disable-next-line: unused-function, missing-fields
  return os.time({ day = date.day, month = date.month, year = date.year, hour = floor(LT), min = math.modf(frac(LT) * 60) })
end

local function getTimezone()
  local now = (os.orgTime or os.time)()
  return os.difftime(now, (os.orgTime or os.time)(os.date("!*t", now)))
end
...
local function sunCalc(time)
  ...
  local date = os.date("*t", time or (os.orgTime or os.time)())
